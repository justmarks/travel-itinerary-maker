...
Current coverage: **975 tests** across 63 test suites.
...
- [x] **Phase 10 — Scheduled / auto email scans** — after a user runs a manual scan once, they can persist it as a recurring schedule (daily / weekly / monthly) from the new "Scheduled scans" panel in `/settings/account` (mobile parity at `/m/settings/account`). Schedules are per-`(provider × folder × frequency)` so a user can watch their personal Gmail "Travel/" label and an Outlook work-folder independently — multiple providers supported. A new `email-scan-schedules` Drizzle migration adds two tables (`email_scan_schedules` + cascade-deleted `email_scan_runs`, capped at the most recent 50 runs per schedule), and a Supabase pg_cron + pg_net job calls a shared-secret-guarded `POST /api/v1/email-scan-schedules/tick` endpoint once a minute. The executor resolves the right connector (Gmail or Microsoft Graph) by `userId × provider`, runs the same `EmailParser` the manual scan flow uses, and lands results in the existing `processed_emails` queue with `parseStatus: "parsed"` — segments still go through human review, same as a manual scan. Push notification fires only when a run produced new segments. New `AutoScanBanner` on `/` and `/m` reads `usePendingEmails()` and surfaces a "review N bookings" pill that opens the existing review sheet. Run history view shows last 50 runs with status + scanned / new counts. Schedules can be paused, edited (provider / folder / frequency change recomputes `nextRunAt`), or deleted from the same panel. One-time Supabase wiring documented in [`docs/auto-email-scan-setup.md`](docs/auto-email-scan-setup.md)
