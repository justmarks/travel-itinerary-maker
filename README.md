...
- [ ] **Create trips from mobile** — segment authoring now lands on `/m` (Phase 4 above), but trip *creation* is still desktop-only. New users hitting `/m` with an empty list get a CTA that bounces them to the desktop create-trip flow. Add a native mobile create-trip flow (title, dates, country / region) so trips can be planned end-to-end on a phone
