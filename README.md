...
Current coverage: **1003 tests** across 65 test suites.
...
- [x] **Phase 10 — Scheduled / auto email scans** — after a user runs a manual scan once, they can persist it as a recurring schedule (daily / weekly) from the new "Scheduled scans" panel in `/settings/account` (mobile parity at `/m/settings/account`). Daily and weekly cadences accept a clock-time anchor (and a day-of-week for weekly) — picked in the user's local zone, stored as UTC `timeOfDay` + `dayOfWeek` so the cron tick honours it without DST drift across the day boundary. Schedules are per-`(provider × folder × frequency)` so a user can watch their personal Gmail "Travel/" label and an Outlook work-folder independently — multiple providers supported. A new `email-scan-schedules` Drizzle migration adds two tables (`email_scan_schedules` + cascade-deleted `email_scan_runs`, capped at the most recent 50 runs per schedule), and a Supabase pg_cron + pg_net job calls a shared-secret-guarded `POST /api/v1/email-scan-schedules/tick` endpoint once an hour. The executor resolves the right connector (Gmail or Microsoft Graph) by `userId × provider`, runs the same `EmailParser` the manual scan flow uses, and lands results in the existing `processed_emails` queue with `parseStatus: "parsed"` — segments still go through human review, same as a manual scan. A per-schedule "include sub-folders / sub-labels" toggle widens the scan to descendants of the picked label (e.g. "Travel" also catches "Travel/Hotels", "Travel/Flights/Confirmed"), expanded at run time via the connector's `listLabels()` so a single setup follows the user's inbox organisation. Push notification fires only when a run produced new segments. New `AutoScanBanner` on `/` and `/m` reads `usePendingEmails()` and surfaces a "review N bookings" pill that opens the existing review sheet. Run history view shows last 50 runs with status + scanned / new counts. Schedules can be paused, edited (cadence / anchor changes recompute `nextRunAt`), or deleted from the same panel. One-time Supabase wiring documented in [`docs/auto-email-scan-setup.md`](docs/auto-email-scan-setup.md)
...
- [x] **Calendar sync on shared trips** — calendar sync no longer hides behind `isOwner`; any user with edit access (the owner or a shared-edit recipient) can push the trip to their OWN Google / Outlook calendar. Per-(trip, user) state lives in a new `trip_user_calendar_syncs` table holding `(tripId, userId, calendarId, segmentEventMap)`, so two users pushing the same trip don't clobber each other's event ids — the owner's next "update" sync still finds the right events, and so does the recipient's. Migration `0007` backfills the new table from the legacy `trips.calendar_id` + `segments.calendar_event_id` columns (each existing trip with `calendar_id` set becomes a sync row attributed to the trip owner); the legacy columns stay around as dead-but-harmless after the backfill. A new `GET /api/v1/trips/:tripId/calendar/sync` returns the requester's sync state for the trip; the `useCalendarSync` hook plus the desktop `TripActionsMenu` and the mobile sync sheet read from it. Owner-only chrome (Share / Delete / Email scan) stays gated on `isOwner` — only the calendar lane widened to `canEdit`
