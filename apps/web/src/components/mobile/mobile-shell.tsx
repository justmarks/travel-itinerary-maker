...
/**
 * Two-state pill segmented control for switching between the Feed and
 * Carousel prototypes. Lives at the bottom of the screen, thumb-reachable.
 */
export function ViewSwitcher({
  ...
  active: "feed" | "carousel";
  feedHref: string;
  carouselHref: string;
}): React.JSX.Element {
  return (
    <div className="sticky bottom-0 z-30 shrink-0 border-t border-border/60 bg-background/90 px-3 pb-[max(env(safe-area-inset-bottom),0.5rem)] pt-2 backdrop-blur">
      <div className="grid grid-cols-2 gap-1 rounded-full border bg-muted p-1 text-sm">
        <Link
          href={feedHref}
        ...
        >
          <span aria-hidden>Alt 1</span>
          <span>Feed</span>
        </Link>
        <Link
          href={carouselHref}
          aria-pressed={active === "carousel"}
          className={cn(
            "flex items-center justify-center gap-1.5 rounded-full px-3 py-1.5 font-medium transition-colors",
            active === "carousel"
              ? "bg-background text-foreground shadow-sm"
