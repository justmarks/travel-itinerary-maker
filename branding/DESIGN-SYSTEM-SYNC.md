...
| 2026-05-04 | (audit) | App-extension layer in `globals.css` grew three families: `--status-{ok,warn,danger,info,attention,muted}-{rail,bg,fg}` for semantic UI states; `--todo-{meals,activities,research,logistics}-{bg,fg}` aliased to the status palette; and a Tailwind v4 `@utility text-kicker` for the `--type-xs` uppercase-metadata pattern. Refactored ~14 components off Tailwind 50/600 status classes (trip-card, email-scan-dialog, html-import-dialog, trip-todos, trip-history, itinerary-day, trip-detail-client, mobile-segment-card, mobile-segment-detail-sheet, mobile-share-sheet, mobile-share-button, mobile-shell, mobile-todos-sheet, mobile-todo-form-sheet, suggest-meals-dialog, share-trip-dialog, shared-trip-client × 2). Lifted the desktop `useCategoryPinColors` hook to `lib/category-pin-colors.ts` and used it in both `map-view.tsx` and `mobile-day-map.tsx` so the desktop and mobile pin palettes can never drift again. |
| 2026-05-06 | `5e5pzlvzJBb8vBfWBNEQwA` | Bundle absorbs the four app-extension blocks that used to live in `globals.css`: ShadCN primitives (`--secondary*`, `--accent*`, `--popover*`, `--destructive-foreground` × light + dark), dark-mode segment lifts (every `--seg-*-bg` / `--seg-*-fg` overridden in `.dark` with translucent 950/60 wash + 300-weight foregrounds), timeline grid sizing tokens (`--timeline-label-col`, `--timeline-day-min` with the 640 / 768 px breakpoints + print override), and the `@media print` block (`* { print-color-adjust: exact }`, `.print-hidden`, timeline overflow / sticky / pill-wrap resets). The `.no-scrollbar` utility also moved into the bundle. **New tokens added:** `--pin-{transport,hotel,activity,dining}` aliasing the matching `--cat-*-fg` so SVG map pins read a single token instead of going through `categoryTokenName(cat)`. Refactor: `useCategoryPinColors` reads `--pin-${cat}` directly; `categoryTokenName` removed (sole consumer was `map-view.tsx`'s legend swatch, which now reads `var(--pin-${cat})` too). `globals.css` is down to Tailwind setup (`@import`, `@custom-variant`, `@theme inline`, `@layer base`), the `text-kicker` utility, the `.flag-font` polyfill class, and the one app-only `.timeline-grid > * { min-width: 0 }` rule that lets pills with `truncate` actually clip. History-icon canon (`trip-history.tsx`'s `KIND_PRESENTATION`) is documented in the bundle's `components-history-icons.html` preview — no code change needed; both sides already share the four-tone (ok / info / danger / muted) status palette. |
| 2026-05-04 | `zbJReNtaNHsYg2JgSGPC2A` | Bundle now ships the full status / todo / category-collapse palettes that previously lived as app extensions: `--status-*` (5 tones — `attention` was retired; the two consumers — `email-scan-dialog`, `html-import-dialog` — remap `enrichment` to `info`), `--todo-*` (4 categories aliased to status), and `--cat-{transport,lodging,activity,dining}-{rail,bg,fg}` for the 4-bucket map / timeline collapse. Segment palette regrouped by category family — transport→blues, lodging→violet, activity→greens, dining→warm; dark-mode lifts in `globals.css` rewritten to track the new hues, and `useCategoryPinColors` SSR fallbacks updated. Visual changes: trip-history rewritten to bundle's `History.jsx` kit (sectioned day groups, disc + body layout); `itinerary-day.tsx` `SegmentRow` style brought up to mobile parity (32×32 tinted icon tile + 4px left rail + `shadow-xs`); timeline pills switched to solid `--cat-*-rail` background with white text; trip-todos add form gained the bundle's `border + bg-card + p-3` card frame. |
