...
export class XlsxTripImporter {
  async parseWorkbook(buffer: Buffer): Promise<ParsedWorkbook> {
    ...
    const days: ParsedWorkbookDay[] = buckets.map((bucket) => {
      ...
      // cruise segments carry the embarkation port as their city so the card
      // still shows a sensible location when the row only says "Board ship".
      ...
      for (const block of transportBlocks) {
        ...
        if (seg.type === "cruise" && dayCity) seg.city = dayCity;
        segments.push(seg);
      }
